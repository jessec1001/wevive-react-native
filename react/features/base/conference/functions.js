...
export function getConferenceName(stateful: Function | Object): string {
  ...
  const flags = state['features/base/flags'];
...
export function getConferenceNameForTitle(stateful: Function | Object) {
  return safeStartCase(
    safeDecodeURIComponent(toState(stateful)['features/base/conference'].room),
  );
}
...
export function updateConferenceDuration(
  stateful: Function | Object,
): function {
  const state = toState(stateful);
...
export function getCurrentConference(stateful: Function | Object) {
  const {conference, joining, leaving, membersOnly, passwordRequired} = toState(
    stateful,
  )['features/base/conference'];
