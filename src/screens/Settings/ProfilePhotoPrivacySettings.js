...
export default function ProfilePhotoPrivacySettings({navigation}) {
  const ctx = React.useContext(ChatContext);
  const selected = ctx.getSetting('ProfilePhotoPrivacy');
  const setSetting = (value) => {
    return () => {
      ctx.setSetting('ProfilePhotoPrivacy', value);
